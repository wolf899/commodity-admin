  ...
  <div>
    <el-form ref="form" label-width="80px">
      ...
      <el-form-item label="价格(元)">
        <el-input v-model="skuInfo.price" placeholder="价格(元素)" type="number"></el-input>
      </el-form-item>
        ...
        <el-table style="width: 100%" border :data="spuImageList" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="prop" label="图片" width="width">
            <template slot-scope="row">
              <img :src="row.row.imgUrl" style="width:100px;height:100px">
            </template>
          </el-table-column>
          ...
          <el-table-column prop="prop" label="操作" width="width">
            <template slot-scope="row">
              <el-button v-if="row.row.isDefault==0" type="primary" @click="changeDefault(row.row)">设置默认</el-button>
...
export default {
  ...
  methods: {
    ...
    cancel(){
      ...
      // 清除数据 this._data:响应式数据  this.$options.data()：data(return{})内的数据
    ...
    async save(){
      ...
      if(result.code === 200){
        this.$message.success({ message:'添加SKU成功' })
        this.$emit('changeScenes', 0)
      }
    }
  }
